...
if __name__ == "__main__":
    ...
    model = Net()
    model.load_state_dict(torch.load("./mymodel_mnist.pth", map_location=torch.device('cpu')))
    ...
    for data, target in tqdm.tqdm(new_test_loader):
        ...
        for layer_index, layer_logits_values in enumerate(logits[:-1]):
            ...
            active_neurons = torch.where(activations > 0.2)[0]
    ...
    fpl_scores = list(filter(lambda item: item[1] > 0.99, fpl_scores))
    faulty_cdps = list(map(lambda item: item[0], fpl_scores))

    with open('./decision_graph.pickle', 'rb') as handle:
