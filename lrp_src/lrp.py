...
from lrp_src.utils import layers_lookup
from lrp_src.lrp_layers import RelevancePropagationIdentity
    ...
    def _create_lrp_model(self) -> torch.nn.ModuleList:
        ...
        for i, layer in enumerate(layers[::-1]):
            try:
                ...
                layers[i] = RelevancePropagationIdentity(layer=layer)
                message = f"Layer-wise relevance propagation not implemented for " \
                          f"{layer.__class__.__name__} layer."
                print(message, "Using RelevancePropagationIdentity instead.")
